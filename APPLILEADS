...
.ffe-header {
    background: linear-gradient(90deg, var(--blu-scuro) 0%, #16305e 100%);
    padding: 22px 30px;
    border-radius: 10px;
...
.ffe-header p {
    color: var(--grigio-metal) !important;
    margin: 4px 0 0 0;
    font-size: 0.95rem;
}
...
.ffe-card {
    ...
    box-shadow: 0 2px 6px rgba(14,31,61,0.06);
    margin-bottom: 14px;
}
...
def init_session_state():
    ...
    if "places_api_key" not in st.session_state:
        st.session_state.places_api_key = ""
...
# =========================================================================
# RICERCA AZIENDE REALI - GOOGLE PLACES REST API (Text Search + Details)
# =========================================================================

def google_places_text_search(query, api_key, max_results=10):
    """Ricerca testuale reale su Google Places. Ritorna (lista_risultati,
    errore). In caso di errore/chiave assente, lista_risultati è vuota e
    errore contiene il motivo, cosi' il chiamante puo' fare fallback mock
    in modo trasparente."""
    if not api_key:
        return [], "no_key"
    ...
    try:
        ...
        if stato not in ("OK", "ZERO_RESULTS"):
            return [], "errore API Places: " + stato
        risultati = data.get("results", [])[:max_results]
        return risultati, None
    except Exception as exc:
        return [], "errore di rete: " + str(exc)


def google_places_details(place_id, api_key):
    """Recupera dettagli reali (sito web, telefono) per un place_id."""
...
def costruisci_query_places(settore, paese):
    parola_chiave = SETTORE_QUERY_PLACES.get(
        settore, "plastic processing manufacturer")
    return parola_chiave + " in " + paese


def lead_da_risultato_places(risultato, settore, paese, fonte,
                              api_key, nota_extra=""):
    """Converte un risultato Google Places reale in un lead dell'app,
    arricchendolo con i Place Details (sito/telefono) quando possibile."""
    lead_id = st.session_state.next_id
    st.session_state.next_id += 1

    dettagli = google_places_details(risultato.get("place_id", ""), api_key)

    nome = dettagli.get("name") or risultato.get("name", "Azienda senza nome")
    indirizzo = dettagli.get("formatted_address") or risultato.get(
        "formatted_address", "")
    sito = dettagli.get("website", "")
    telefono = dettagli.get("formatted_phone_number") or dettagli.get(
        "international_phone_number", "")
    rating = dettagli.get("rating") or risultato.get("rating")
    n_recensioni = dettagli.get("user_ratings_total") or risultato.get(
        "user_ratings_total", 0)

    punteggio = int(min(99, max(35, (rating or 3.5) * 20)))
    nota_rating = ""
    if rating:
        nota_rating = "Google rating: " + str(rating) + " (" \
            + str(n_recensioni) + " recensioni). "
    nota_completa = (nota_rating + "Indirizzo: " + indirizzo
    ...
    lead = {
        ...
        "paese": paese,
        "settore": settore,
        "dimensione": "Da verificare (dato reale non disponibile)",
        ...
        "email": "Non disponibile - vedi sito web" if not sito
                  else "Verificare su " + sito,
...
def cerca_aziende_reali(settore, paese, fonte, n_lead=10, query_extra=""):
    """Wrapper di alto livello: esegue una ricerca reale su Google Places
    per un settore/paese e ritorna una lista di lead reali. Ritorna una
    lista vuota + messaggio di errore se la chiave manca o la chiamata
    fallisce, per permettere un fallback mock trasparente al chiamante."""
    api_key = st.session_state.places_api_key.strip()
    query = costruisci_query_places(settore, paese)
    if query_extra:
        query = query + " " + query_extra

    risultati, errore = google_places_text_search(query, api_key,
                                                    max_results=n_lead)
    if errore:
        return [], errore
    if not risultati:
        return [], "nessun risultato reale trovato per questa query"

    leads = []
    for r in risultati:
        leads.append(lead_da_risultato_places(r, settore, paese, fonte,
                                               api_key))
    return leads, None
...
def badge_fonte_dati(dati_reali):
    if dati_reali:
        return ('<span class="ffe-badge ffe-badge-info">✅ DATI REALI '
                 '(Google Places)</span>')
    return ('<span class="ffe-badge ffe-badge-bassa">⚠️ DATI DIMOSTRATIVI '
             '(mock)</span>')
...
def sidebar():
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("#### 🌍 Ricerca aziende REALI")
        st.session_state.places_api_key = st.text_input(
            "Google Places API Key",
            value=st.session_state.places_api_key,
            type="password",
            help="Necessaria per trovare aziende vere (Ricerca per "
                 "Settore, Lookalike, Sales Twin). Senza chiave, questi "
                 "moduli usano dati dimostrativi (mock) chiaramente "
                 "segnalati. Ottienila su console.cloud.google.com "
                 "abilitando 'Places API'.",
        )
        if st.session_state.places_api_key:
            st.success("🔎 Ricerca aziende REALI attiva")
        else:
            st.warning("⚠️ Nessuna chiave: ricerca aziende in modalita' "
                        "dimostrativa (mock)")
        ...
        st.session_state.gemini_api_key = st.text_input(
            ...
            type="password",
            help="Se vuota, l'app usa generazione dimostrativa (mock) "
                 "trasparente per dossier ed email.",
        )
        if st.session_state.gemini_api_key:
            st.success("Modalita' AI live attiva")
        else:
            st.info("Modalita' dimostrativa (dati mock)")

        st.markdown("---")
        st.caption(
            "ℹ️ Nota onestà tecnica: solo i moduli 'Ricerca per Settore', "
            "'Analisi Lookalike' e 'Sales Twin Verticale' possono usare "
            "dati reali (Google Places). I moduli 'Ricerca Fiere', "
            "'Progetti di Investimento' e 'Radar Espansione' restano in "
            "modalita' dimostrativa: non esiste una fonte gratuita "
            "affidabile per elenchi espositori fiere o notizie di "
...
def pagina_ricerca_settore():
    ...
    if submit:
        ...
        usa_dati_reali = bool(st.session_state.places_api_key.strip())
        nuovi = []
        errori = []

        if usa_dati_reali:
            with st.spinner("Ricerca aziende reali su Google Places..."):
                ...
                for settore in settori_sel:
                    for paese in paesi_sel:
                        trovati, errore = cerca_aziende_reali(
                            settore, paese, "Ricerca per Settore (reale)",
            ...
            for _ in range(n_lead):
                nuovi.append(genera_lead_mock(
                    ...
                    urgenza=random.choice(LIVELLI_URGENZA),
                    fonte="Ricerca per Settore (mock)",
                ))
        ...
        badge = badge_fonte_dati(usa_dati_reali)
        st.markdown(badge, unsafe_allow_html=True)
        st.success(str(len(nuovi)) + " lead aggiunti al database.")
        colonne_mostra = ["azienda", "paese", "settore", "urgenza",
                            "punteggio", "sito_web", "telefono"] \
            if usa_dati_reali else ["azienda", "paese", "settore",
                                      "dimensione", "urgenza", "punteggio"]
...
def pagina_lookalike():
    ...
    if submit:
        ...
        if usa_dati_reali:
            ...
            for _ in range(n_lead):
                nuovi.append(genera_lead_mock(
                    ...
                    fonte="Lookalike di: " + cliente_rif + " (mock)",
        ...
        st.success(str(len(nuovi)) + " aziende gemelle di '" + cliente_rif
                    + "' individuate.")
        colonne_mostra = ["azienda", "paese", "settore", "punteggio",
